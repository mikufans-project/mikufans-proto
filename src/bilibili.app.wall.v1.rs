...
pub mod wall_server {
    ...
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with WallServer.
    #[async_trait]
    pub trait Wall: std::marker::Send + std::marker::Sync + 'static {
        ///
        async fn rule_info(
            &self,
            request: tonic::Request<super::RuleRequest>,
        ) -> std::result::Result<tonic::Response<super::RulesReply>, tonic::Status>;
    }
    ///
    #[derive(Debug)]
    pub struct WallServer<T> {
        inner: Arc<T>,
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for WallServer<T>
    where
        T: Wall,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/bilibili.app.wall.v1.Wall/RuleInfo" => {
                    ...
                    struct RuleInfoSvc<T: Wall>(pub Arc<T>);
                    impl<T: Wall> tonic::server::UnaryService<super::RuleRequest>
                    for RuleInfoSvc<T> {
                        type Response = super::RulesReply;
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as Wall>::rule_info(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    pub const SERVICE_NAME: &str = "bilibili.app.wall.v1.Wall";
    impl<T> tonic::server::NamedService for WallServer<T> {
